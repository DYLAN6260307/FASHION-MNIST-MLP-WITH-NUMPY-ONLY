...
本仓库实现了一个只依赖 NumPy 的三层 MLP，用于完成 Fashion-MNIST 十分类作业。代码没有使用 PyTorch、TensorFlow、JAX 等自动微分框架，而是在 `fashion_mlp/autodiff.py` 中手写线性层、激活函数、交叉熵和反向传播。
...
- `fashion_mlp/model.py`: 三层 MLP 分类器，支持隐藏层大小和激活函数配置，支持模型权重保存/加载。
...
```bash
python scripts/train.py --epochs 20 --hidden-dim 128 --activation relu --learning-rate 0.05 --lr-decay 0.95 --weight-decay 1e-4
```
...
```bash
python scripts/hyperparam_search.py --epochs 5 --max-train 12000 --max-val 2000
```
